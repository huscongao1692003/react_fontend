...
function CreateLesson({ courseData, setCourseData, courseId, setRefresh, refresh }) {
      ...
      try {
        ...
        if (response.data.status === "OK") {
          ...
          const maxNumber = response.data.data.reduce(
            (max, topic) => Math.max(max, topic.number),
            0
          );
            ...
            <form>
              <div className="mb-3">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTopicData({ ...topicData, topicTitle: e.target.value })
                  }
                />
              </div>
              <div className="mb-3">
